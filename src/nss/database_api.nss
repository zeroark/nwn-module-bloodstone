...
//Highly recommended to use with SQL string data.
...
void dbLogMsg(string sMessage, string sType = "error", int nTRUEID = 0, int nINDEXID = 0, int nLIID = 0, int nPLID = 0, string sTable = "logmsg");
...
//call this only once (client enter). Instead, use dbGetTRUEID(oPC).
...
//Terminates the session in the db.
void dbSessionEnd();
...
//Fetches the LocalInt "INDEXID". This should be defined in the dbInitTRUEID function.
//Returns 0 if "INDEXID" or "TRUEID" ints are 0.
...
void dbLogMsg(string sMessage, string sType="error", int nTRUEID=0, int nINDEXID=0, int nLIID=0, int nPLID=0, string sTable = "logmsg")
{
    NWNX_SQL_ExecuteQuery("insert into " + sTable + " (msg,type,trueid,dexid,liid,plid) values (" +
        DelimList(dbQuotes(sMessage), dbQuotes(sType), IntToString(nTRUEID), IntToString(nINDEXID), IntToString(nLIID), IntToString(nPLID)) + ")");
...
int dbGetCKID(object oPC)
{
    if (!GetIsObjectValid(oPC)) 
        return 0;   //If the oPC was booted/left, returns

    int nCKID = GetLocalInt(oPC,"CKID");
    if (!nCKID)   // NO VARIABLE YET, CREATE IT - PLAYER JUST LOGGED IN
    {
        string sSQL;
...
int dbGetIPID(object oPC)
{
    ...
    if (!nIPID)   // NO VARIABLE YET, CREATE IT - PLAYER JUST LOGGED IN
    {
        string sSQL;
...
int dbGetPLID(object oPC)
{
    ...
    if (!nPLID)   // NO VARIABLE YET, CREATE IT - PLAYER JUST LOGGED IN
    {
        string sSQL;
...
int dbGetACID(object oPC)
{
    ...
    if (!nACID)   // NO VARIABLE YET, CREATE IT - PLAYER JUST LOGGED IN
    {
        string sSQL;
...
int dbGetLIID(object oPC)
{
    ...
    if (!nLIID)   // NO VARIABLE YET, CREATE IT - PLAYER JUST LOGGED IN
    {
        ...
        //Grabs the generated new login id (login index)
...
int dbInitTRUEID(object oPC)
{
    ...
    if (!nTRUEID)  //Search db and fetch/create a TRUEID
    {
        ...
        int nGenerateINDEXID = FALSE;   //Because I cannot know the TRUEID until searching is finished, it will add dexid entry after.
        ...
        if (NWNX_SQL_ReadyToReadNextRow())
        {
            ...
            nINDEXID  = StringToInt(NWNX_SQL_ReadDataInActiveRow(1));
        }
        else
        {   //Search for any of the credentials in the DB
            ...
            */
            ...
            if (nGenerateINDEXID)
            {
                string sGenerateINDEXID  = "insert into dexid (trueid, acid, ckid, ipid) values (" + DelimList(IntToString(nTRUEID), sACID, sCKID, sIPID) + ")";
                NWNX_SQL_ExecuteQuery(sGenerateINDEXID);
                NWNX_SQL_ExecuteQuery("select last_insert_id() from dexid limit 1");
                if (NWNX_SQL_ReadyToReadNextRow())
                {
                    NWNX_SQL_ReadNextRow();
                    nINDEXID = StringToInt(NWNX_SQL_ReadDataInActiveRow(0));
                }
            }
            if (!nTRUEID)
            {
                ...
                {
                    SetCutsceneMode(oPC);
                    ActionFloatingTextStringOnCreature("DM TRUEID FAILED. GTFO or contact an admin on Discord. Enjoy Chapter 1.", oPC);
                    //ActivatePortal(oPC, "jail.dungeonternalx.com:5123","","",TRUE); // jail server
                }
                else
                    dbBootPC(oPC,"TRUEID System Verification Failed. Contactan admin on Discord.  \n", sName + " || " + 
                            sCDKEY + " failed to validate TRUEID", 8.0f);
                return 0;
            }
        }
        if (nTRUEID) SetLocalInt(oPC, "TRUEID", nTRUEID);
        if (nINDEXID)  SetLocalInt(oPC, "INDEXID", nINDEXID);
    }
    SetLocalInt(oPC, "TRUEID", nTRUEID);
    //SendMessageToPC(oPC,IntToString(GetLocalInt(oPC,"TRUEID")) + "   " + IntToString(GetLocalInt(oPC,"INDEXID")));
    return nTRUEID;
}
...
void dbCheckBan(object oPC)
{
    ...
    if (NWNX_SQL_ReadyToReadNextRow())
    {
        ...
        if (StringToInt(NWNX_SQL_ReadDataInActiveRow(0)))
        {
            ...
            dbBootPC(oPC, "You are perma-banned. Contact an admin if you feel this was a mistake.", "Perma-banned player joining:" + dbPCtoString(oPC), 10.0f);
        }
    }
    //Check Temp
    ...
    if (NWNX_SQL_ReadyToReadNextRow())
    {
        ...
        SetLocalInt(oPC, "BANNED", TRUE);
        SetLocalInt(oPC, "LOGOUT", LOGOUT_TYPE_BANNED);
        dbBootPC(oPC,"You are temp-banned. Contact an admin if you feel this was a mistake.", "Temp-banned player joining. Expires " + 
                NWNX_SQL_ReadDataInActiveRow(0) + dbPCtoString(oPC), 10.0f);
    }
}

void dbApplyBan(object oPC, int nBanType=BAN_TYPE_TEMP, int nBanLength = 1, string sReason = "")
{
    int nTRUEID = dbGetTRUEID(oPC); // GET TRUEID
    string sSQL = "";
    string sTime;
    switch (nBanType)
    {
        case BAN_TYPE_TEMP:       //adds ban to temporaryban table
        {
            ...
            sSQL  = "insert into temporaryban (trueid, expires, reason) values (" + IntToString(dbGetTRUEID(oPC)) + 
                    "DATE_ADD(NOW(), INTERVAL " + IntToString(nBanLength) + " HOUR)," + dbQuotes(sReason) + ")";
...
{ // CALL THIS AS OFTEN AS NEEDED
    ...
    if (sActive != "0")
        sActive = IntToString(GetLocalInt(GetModule(), "SERVER"));
...
int dbGetINDEXID(object oPC)
{
    if (!GetIsObjectValid(oPC)) //If the oPC was booted/left, returns
        return 0;

    int nINDEXID = GetLocalInt(oPC, "INDEXID");
    if (!nINDEXID || !dbGetTRUEID(oPC))   
        return 0;
...
string dbGetTRUEIDName(object oPC)
{
    if (GetLocalString(oPC, "TRUEIDNAME") != "") 
    ...
    else
    {
        ...
        {
            dbBootPC(oPC, "Could not fetch your TRUEID Name.", "Player kicked for not having a TRUEID name. Usually meanys the DB is down.");
            return "Error";
        }
    }
}
...
// and forth to the database, and converted back to the proper
// form when retrieved.
///////////////////////////////////////////////////////////////////////////////
...
vector dbtringToVector(string sVector)
{
    float fX, fY, fZ;
    int iPos, iCount;
